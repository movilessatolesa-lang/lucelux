...
export default function PresupuestosPage() {
  ...
  const [trabajosPresupuestoIds, setTrabajosPresupuestoIds] = useState<Set<string>>(new Set());
  const [convirtiendo, setConvirtiendo] = useState<string | null>(null);

  useEffect(() => {
    Promise.all([getPresupuestos(), getClientes(), getTrabajos()]).then(([ps, cs, ts]) => {
      setPresupuestos(ps);
      setClientes(cs);
      const ids = new Set(ts.filter((t) => t.presupuestoId).map((t) => t.presupuestoId!));
      setTrabajosPresupuestoIds(ids);
    });
  }, []);
  ...
  const enlacePresupuesto = (p: typeof presupuestos[0]) => {
    if (typeof window === "undefined") return "";
    // La página pública busca por url_firma (token), no por id interno
    const token = p.urlFirma ?? p.id;
    return `${window.location.origin}/presupuestos/${token}/cliente`;
  };
  ...
  const waUrlPresupuesto = (p: typeof presupuestos[0]) => {
    ...
    const enlace = enlacePresupuesto(p);
  ...
  const handleConvertirATrabajo = async (p: Presupuesto) => {
    if (trabajosPresupuestoIds.has(p.id)) return;
    setConvirtiendo(p.id);
    try {
      const adelanto = p.porcentajeAdelanto > 0
        ? Math.round(p.importeTotal * p.porcentajeAdelanto / 100 * 100) / 100
        : 0;
      await createTrabajo({
        ...
        notas: p.notas ?? "",
        estado: "aprobado",
        estadoCobro: adelanto > 0 ? "sin_adelanto" : "sin_adelanto",
      });
      setTrabajosPresupuestoIds((prev) => new Set([...prev, p.id]));
    } catch {
  ...
  const handleEnviarCliente = async (p: Presupuesto) => {
    ...
    const wa = waUrlPresupuesto({ ...p, estado: "enviado" });
    if (wa) {
            ...
            >
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex items-center gap-2">
                  ...
                  {p.estado === "aceptado" && trabajosPresupuestoIds.has(p.id) && (
                ...
                <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                  ...
                  {p.estado !== "enviado" && p.estado !== "aceptado" && p.estado !== "rechazado" && (
                    <button
                  ...
                  {p.estado === "aceptado" && !trabajosPresupuestoIds.has(p.id) && (
                    <button
                      onClick={() => handleConvertirATrabajo(p)}
                      disabled={convirtiendo === p.id}
                      className="px-3 py-2 text-xs font-medium bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-lg transition-colors disabled:opacity-50"
                    >
                      {convirtiendo === p.id ? "Creando..." : "🔧 Pasar a trabajo"}
                    </button>
                  )}
                  <button
